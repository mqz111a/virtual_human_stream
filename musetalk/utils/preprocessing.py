...
def get_face_landmark(face_land_mark, upperbondrange):
    ...
    f_landmark = (
        np.min(face_land_mark[:, 0]), int(upper_bond), np.max(face_land_mark[:, 0]), np.max(face_land_mark[:, 1]))
    return f_landmark
...
def get_landmark_and_bbox(img_list, upperbondrange=0):
    ...
    for fb in tqdm(batches):
        ...
        for j, f in enumerate(bbox):
            ...
            f_landmark = get_face_landmark(face_land_mark, upperbondrange)
