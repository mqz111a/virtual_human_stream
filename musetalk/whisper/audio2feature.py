...
class Audio2Feature():
    ...
    def get_feature_slice(self, feature_array, idx, length):
        idx = max(0, idx)
    ...
    def get_sliced_feature(self,
        ...
        for idx in range(left_idx, right_idx):
            x, idx = self.get_feature_slice(feature_array, idx, length)
    ...
    def get_sliced_feature_sparse(self, feature_array, vid_idx, audio_feat_length=[2, 2], fps=25):
        ...
        for dt in range(-audio_feat_length[0], audio_feat_length[1] + 1):
            ...
            if left_idx < 1 or left_idx > length - 1:
                ...
                for idx in range(left_idx - 1, left_idx + 1):
                    x, idx = self.get_feature_slice(feature_array, idx, length)
                    selected_feature.append(x)
                    selected_idx.append(idx)
    ...
    def transcode_audio(self, audio_path):
    ...
    def audio2feat(self, audio_path):
        return self.transcode_audio(audio_path)

def test_audio2feature():
    ...
    # Convert the audio to features
    array = audio_processor.audio2feat(audio_path)
    print(f"Shape of the feature array: {array.shape}")
